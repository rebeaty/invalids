#!/usr/bin/env python3
"""
Invalid Response Detection for Creativity Assessments (v4)
===========================================================
...
Usage
-----
  python score_invalid_v4.py --task all --limit 200   # Quick test
  python score_invalid_v4.py --task all --limit 0     # Full run
  python score_invalid_v4.py --task meta --skip-ppl   # Single task

Output
------
  - Per-task: {task}/{task}_scored_augmented_v4.csv
  - Summary: results_summary_augmented_v4.csv
...
# =============================================================================
# TASK-SPECIFIC 1-5 QUALITY PROMPTS
# =============================================================================
...
Rate the validity from 1 to 5:
  ...
  5 = Excellent - a well-formed metaphor that completes the prompt naturally
...
def main():
    parser = argparse.ArgumentParser(description="Score creativity responses (v4 - task-specific 1-5 quality)")
    ...
    print("=" * 60)
    print("Invalid Detection v4 - Task-Specific 1-5 Quality Prompts")
    print("=" * 60)
    ...
    for task in tasks:
        ...
        if run_gemini and GEMINI_API_KEY:
            ...
            if client:
                ...
                # Show distribution
                print(f"  Quality distribution: {df['gemini_quality'].value_counts().sort_index().to_dict()}")
        ...
        if run_gemini:
            # Quality score: higher = valid, so higher_is_better=True for detecting invalids
            gem = evaluate(df["label"], df["gemini_quality"], higher_is_better=True)
            print_result("Gemini Quality (1-5)", gem)
            all_results.append({"task": task, "dataset": dataset_type, "metric": "gemini", **gem})
            
            # Show mean by label
            print(f"  Mean quality by label:")
        ...
        # Save
        out_path = script_dir / task / f"{task}_scored_{dataset_type}_v4.csv"
    ...
    if all_results:
        ...
        summary_path = script_dir / f"results_summary_{dataset_type}_v4.csv"
